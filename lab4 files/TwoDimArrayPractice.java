   ...
   public int countFound( int value )
   {
    ...
    // Write your code here:
	    ...
	    for ( int row = 0; row < intArray.length; row++ )
	    {
	        ...
	        for ( int column = 0; column < intArray[row].length; column++ )
	        {
	          if(intArray[row][column]==dum)
	        	  lum++;
	          animate( row, column, dum );  // needed to create visual feedback
	        }
	        System.out.println(intArray);
	    }

     return lum; // replace this line with your return statement
   }
